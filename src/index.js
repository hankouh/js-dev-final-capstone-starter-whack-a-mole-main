...
const startButtonEasy = document.querySelector('#startEasy');
const startButtonNormal = document.querySelector('#startNormal');
const startButtonHard = document.querySelector('#startHard');
...
function chooseHole(holes) {
  ...
  let randIndex = randomInteger(0,8);
...
//  updateMsg("toggle called " + hole.id)
  return hole;
}
...
function startGame(diffLevel){
  ...
  // if game is running do nothing when start is pressed or hide the start button
  if (time > 0 ) {
    updateMsg("Game started, keep playing!");
  return "game started";
  }
  ...
  updateMsg("Game started! ("+diffLevel+")");
  return "game started";
}
...
startButtonEasy.addEventListener("click", startGameEasy);
startButtonNormal.addEventListener("click", startGameNormal);
startButtonHard.addEventListener("click", startGameHard);
